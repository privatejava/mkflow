...
public class AWSService {
    ...
    public IntStream getIntStream(List<Map> lastLines,List<List<ResultField>> finalResults){
        return IntStream
            ...
            .filter(i -> {
                List<ResultField> m = finalResults.get(i);
                ...
                if (msg.isPresent() && time.isPresent()) {
                    List<Map> msgFound = lastLines.stream().filter(l -> l.get("message").toString().equalsIgnoreCase(msg.get().value())).collect(Collectors.toList());
                    List<Map> timeFound = lastLines.stream().filter(l ->
                        msgFound.stream().filter(mm -> mm.get("time").toString().equalsIgnoreCase(l.get("time").toString()))
                            .findAny().isPresent())
                        .collect(Collectors.toList());
                    if (!msgFound.isEmpty() && timeFound.size() > msgFound.size()) {
                        return true;
                    }
                }
                return false;
     ...
     * @return
     */
    public CompletableFuture<List<LogMessage>> getAWSCloudwatchResult(List<Map> lastLines, String uniqueId, Long from) {
        CompletableFuture<List<LogMessage>> completeFuture = new CompletableFuture<>();
        Utils.getExecutorService().submit(() -> {
            CloudWatchLogsClient client = CloudWatchLogsClient.builder().region(Region.of(System.getenv("AWS_REGION"))).httpClient(
            ...
            List<List<ResultField>> results = queryRes.results();
